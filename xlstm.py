...
class BlockDiagonal(nn.Module):
    def __init__(self, in_features, out_features, num_blocks):
        ...
        self.blocks = nn.ModuleList([
            nn.Linear(block_in_features, block_out_features).to("cuda") for _ in range(num_blocks)
        ])

    def forward(self, x):
        x = x.chunk(self.num_blocks, dim=-1)
        x = [block(x_i.to("cuda")) for block, x_i in zip(self.blocks, x)]
        x = torch.cat(x, dim=-1)
        return x
...
class sLSTMBlock(nn.Module):
    ...
    def forward(self, x, prev_state):
        ...
        h_prev, c_prev, n_prev, m_prev = (s.to("cuda") for s in prev_state)
...
class sLSTM(nn.Module):
    ...
    def __init__(self, input_size, hidden_size, num_heads, num_layers=1, batch_first=False, proj_factor=4 / 3):
        ...
        self.layers = nn.ModuleList([sLSTMBlock(input_size, hidden_size, num_heads, proj_factor).to("cuda") for _ in range(num_layers)])
    ...
    def forward(self, x, state=None):
        ...
        if state is not None:
            state = torch.stack(list(state)).to("cuda")
            assert state.ndim == 4
            ...
            state = torch.zeros(self.num_layers, 4, batch_size, self.hidden_size,device="cuda")
...
# TODO: Add bias, dropout, bidirectional
    def __init__(self, input_size, hidden_size, num_heads, num_layers=1, batch_first=False, proj_factor=2):
        ...
        self.batch_first = batch_first
        self.proj_factor_slstm = proj_factor
        self.layers = nn.ModuleList([mLSTMBlock(input_size, hidden_size, num_heads, proj_factor).to("cuda") for _ in range(num_layers)])
...
class xLSTM(nn.Module):
    # TODO: Add bias, dropout, bidirectional
    def __init__(self, input_size=128, hidden_size=64, num_heads=4, layers=None, batch_first=False, proj_factor_slstm=4 / 3,
                    proj_factor_mlstm=2):
        ...
        for layer_type in layers:
            if layer_type == 's':
                layer = sLSTMBlock(input_size, hidden_size, num_heads, proj_factor_slstm).to("cuda")
            elif layer_type == 'm':
                layer = mLSTMBlock(input_size, hidden_size, num_heads, proj_factor_mlstm).to("cuda")
            else:
